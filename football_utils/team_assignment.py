...
def extract_jersey_roi(frame, detection):
    ...
    torso_y2 = int(y1 + height * 0.55)  # Above waist
    torso_x1 = int(x1 + width * 0.15)   # Avoid arms
    torso_x2 = int(x2 - width * 0.15)   # Avoid arms
...
def is_referee(roi):
    ...
    # Check for dark, desaturated colors (characteristic of black/referee)
    dark_mask = cv2.inRange(hsv_roi,
                          np.array([0, 0, 0]),
                          np.array([180, 30, 40]))  # Very strict for truly black

    dark_ratio = cv2.countNonZero(dark_mask) / roi.size * 3  # Normalize by channels

    # Return True if enough dark pixels
    return dark_ratio > 0.8  # Higher threshold to avoid misclassifying blue
...
def assign_teams(frame, detections, visualize=False):
    ...
    if visualize and player_info:
        # We would implement visualization code here, but since we're 
        # focusing on functional code, this is omitted in this update
        pass

    return final_assignments, team_colors