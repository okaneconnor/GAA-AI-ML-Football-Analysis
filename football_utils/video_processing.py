...
# from football_utils.tracking import calculate_optical_flow, apply_perspective_transform # Not used
...
YOLO_MODEL_NAME = 'yolo11n.pt'      # <--- Changed back to YOLOv11 nano
...
def has_player_movement_pattern(track_history_deque, min_window=5):
    ...
    if len(track_history_deque) < min_window:
        return True
    velocities = []
    track_list = list(track_history_deque)
...
def is_valid_team_color(color, team_centers, color_threshold, team_colors_array):
    ...
    closest_team_idx = 0 if dist0 <= dist1 else 1
        ...
        for i, box in enumerate(detections_with_tracking.xyxy):
            ...
            if track_id in is_static:
                ...
                if not is_moving_like_player:
                    ...
                    if classifier_trained and team_classifier is not None:
                        ...
                        if jersey_roi is not None:
                            ...
                            if hist_features is not None:
                                ...
                                except Exception: pass # Ignore classifier errors
                    ...
                    if not classification_success and assigned_label == "Filtered" and len(current_team_centers) == 2:
                        ...
                        if smoothed_color:
                            is_valid, assigned_cluster = is_valid_team_color(
                                smoothed_color, current_team_centers, max_color_distance_threshold, team_colors_array
                            )
                    ...
                    # Set final display color and label
            ...
            # Drawing Logic (Ellipse and Text)
            ...
            cv2.putText(output_frame, display_label,
                        (center_x - 30, bottom_y + ellipse_axes[1] + 15),
...
def process_video(input_video_path, output_folder):
    ...
    print(f"{log_prefix}Using YOLO Model: {YOLO_MODEL_NAME}, Confidence: {CONFIDENCE_THRESHOLD}") # Reflects v11 change
    start_total_time = time.time()

    # --- Initialization ---
    model = None; cap = None; video_writer = None
    ...
    team_training_data = []
    # Consider adjusting LogisticRegression parameters if needed
    team_classifier = LogisticRegression(multi_class='ovr', solver='liblinear', random_state=42)
    classifier_trained = False

    # --- Main Try Block ---
    try:
        # 1. Load Model (Using the updated constant)
        # Pass the model name/path. Adjust path logic if needed (e.g., '/content/models/yolo11n.pt')
        model_load_path = YOLO_MODEL_NAME
        # If your model isn't in the default ultralytics path, specify full path:
        # model_load_path = f"/content/models/{YOLO_MODEL_NAME}" # Example for Colab
        model = load_yolo_model(model_load_path)
        if model is None: raise RuntimeError("Failed to load YOLO model.")
        ...
        fps = int(fps_read) if fps_read > 0 else 30
        ...
        output_filename = f"{os.path.splitext(os.path.basename(input_video_path))[0]}_output_v11.mp4" # Indicate v11 in name
        ...
        while True:
                ...
                for i in valid_track_indices:
                    ...
                    if len(track_history[track_id]) >= MOVEMENT_WINDOW_SIZE:
                        ...
                        if displacement < MOVEMENT_STATIC_THRESHOLD:
                            is_static.add(track_id); is_track_static = True
                        else:
                            is_static.discard(track_id); current_moving_tracker_ids.add(track_id)
                    ...
                    if not is_track_static and is_in_field_region(box, frame_height, frame_width, FIELD_REGION):
                        ...
                        if roi is not None:
                            if is_referee(roi):
                                is_player_referee[track_id] = True; team_assignments[track_id] = "Referee"
            ...
            for old_id in disappeared_ids:
                 ...
                 team_assignments.pop(old_id, None); is_static.discard(old_id)
                    ...
                    if (tid not in is_player_referee and tid not in is_static and
                        ...
                        for i_f, tid_check in enumerate(detections_with_tracking.tracker_id or []):
                             if tid_check is not None and int(tid_check) == tid:
                                 ...
                                 if is_in_field_region(box_f, frame_height, frame_width, FIELD_REGION): track_found_in_frame = True
                                 break
                        if track_found_in_frame:
                            smoothed_color = get_smoothed_color(player_colors_history[tid])
                            if smoothed_color is not None:
                                colors_for_clustering.append(smoothed_color); tracks_for_clustering.append(tid)
                ...
                if len(colors_for_clustering) >= 4:
                    ...
                    if len(unique_colors) >= 2:
                        try:
                            ...
                            if color_distance(new_team_centers[0], new_team_centers[1]) > 30: # Basic validity check
                                current_team_centers = new_team_centers; print(f"K-Means OK. Centers: {current_team_centers}")
                            else: print("Warning: K-Means centers too close, not updating.")
                            if current_team_centers:
                                c0, c1 = current_team_centers[0], current_team_centers[1]
                                ratio0 = c0[2] / (c0[0] + 1.0); ratio1 = c1[2] / (c1[0] + 1.0)
                                ...
                                # Re-assign based on K-Means distance & populate validation arrays
                                kmeans_labels = kmeans.predict(cluster_colors_np)
                                for i_k, tid_k in enumerate(tracks_for_clustering):
                                    assigned_cluster = kmeans_labels[i_k]; smoothed_color = colors_for_clustering[i_k]
                                ...
                                for tid_assign in current_moving_tracker_ids:
                                     ...
                                     in_field_now = False # Check field pos again
                                     for i_fc, tid_check_fc in enumerate(detections_with_tracking.tracker_id or []):
                                         if tid_check_fc is not None and int(tid_check_fc) == tid_assign:
                                             box_fc = detections_with_tracking.xyxy[i_fc]; in_field_now = is_in_field_region(box_fc, frame_height, frame_width, FIELD_REGION); break
                ...
                if len(team_training_data) >= 10:
                    ...
                    if 0 in labels_in_data and 1 in labels_in_data:
                        try:
                            ...
                            classifier_trained = True; print("Team classifier trained.")
                        except Exception as e_train: print(f"Error training classifier: {e_train}"); classifier_trained = False
                    else: print("Not enough diversity (need samples for label 0 and 1).")
            ...
            if frame_num % 30 == 0 or frame_num < 10:
                ...
                moving_ids_count = len(current_moving_tracker_ids) # Count moving non-static IDs
    ...
    # (Same as before)
        ...
        if cap: cap.release(); print(f"{log_prefix}Video capture released.")
        if video_writer: video_writer.release(); print(f"{log_prefix}Video writer released.")
        ...
        if output_video_path and os.path.exists(output_video_path) and os.path.getsize(output_video_path) > 1024:
            ...
            return output_video_path, status_message
             ...
             status_message = "Error: Output file likely corrupt."; return None, status_message
        else:
             print(f"{log_prefix}Error: Output file not found or processing failed.")
             status_message = "Error: Output file not created."; return None, status_message